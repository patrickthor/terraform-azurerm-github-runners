...
# State migration: these moved blocks prevent Terraform from destroying and
# recreating role assignments that were renamed for clarity.
# Safe to remove after all environments have applied once with these blocks.
moved {
  from = azurerm_role_assignment.runner_pull
  to   = azurerm_role_assignment.runner_acr_pull
}

moved {
  from = azurerm_role_assignment.runner_pull_workload
  to   = azurerm_role_assignment.runner_workload
}
...
locals {
  ...
  # Derive names from aci_name prefix for resources that don't need global uniqueness
  name_suffix = trimprefix(var.aci_name, "ci-")
...
resource "azurerm_user_assigned_identity" "runner_pull" {
  ...
  location            = var.location
  resource_group_name = var.resource_group_name
  tags                = merge(var.tags, { Purpose = "DynamicRunnerACRPull" })
...
resource "azurerm_role_assignment" "scaler_contributor" {
  scope                = var.create_resource_group ? azurerm_resource_group.this[0].id : "/subscriptions/${data.azurerm_client_config.current.subscription_id}/resourceGroups/${var.resource_group_name}"
...
resource "azurerm_monitor_diagnostic_setting" "servicebus" {
  name                       = "diag-${var.servicebus_namespace_name}"
